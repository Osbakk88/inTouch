  ...
  return posts
    .map((post) => {
      ...
      return `<article class="bg-white rounded-lg shadow-md p-4 mb-4 post-card">
        ...
        <p class="text-sm text-gray-500 mt-2">Posted by: <a class="text-rose-700 hover:underline" href="./profile.html?name=${post.author?.name ?? ""}">${post.author?.name ?? "Unknown user"}</a></p>
