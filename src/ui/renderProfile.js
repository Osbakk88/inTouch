...
export function renderProfile(profile = {}) {
  ...
  return `
      ...
      <h2 class="text-2xl font-bold text-rose-700">${profile.name ?? "Unknown user"}</h2>
      <p class="text-gray-600 text-center">${bio}</p>
      <div class="flex gap-3 profile-actions">
      <button class="follow-btn bg-rose-700 text-white rounded-md px-4 py-2 hover:bg-rose-800" data-username="${profile.name}">Follow</button>
      ...
      <p id="follow-feedback" class="text-sm text-rose-700"></p>
    </section>
  `;
}
  ...
  const items = posts
    .map(
      ...
      <article class="post-card border-b border-gray-200 py-3">
        ...
        <a class="view-post-btn text-rose-700 hover:underline" href="./post.html?id=${post.id}">View post</a>
      </article>
    `,
    )
    .join("");
